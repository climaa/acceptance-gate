...
describe('SiteFooter', () => {
  ...
  it('renders whatever year the prop says, reading no clock', () => {
    render(<SiteFooter year={1999} copyright="Carles Lima" links={links} />);

    screen.getByText('© 1999 Carles Lima');

    // Guards against a `new Date()` call sneaking back in: if the component ever
    // reads the clock, this spy catches it even though no assertion above would.
  ...
  it('carries only the block class when no className is supplied', () => {
    const { container } = render(
      <SiteFooter year={2026} copyright="Carles Lima" links={[]} />,
    );
