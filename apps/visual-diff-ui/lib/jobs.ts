...
// The layout and the confinement check every path here passes through.
import { reportDir, setDir, setsFilePath, within } from './paths';
// What a job is, as data. Split out so the three client components that render a
// `HistoryRecord` can name it without importing this file — see lib/job-contract.ts.
import {
  type HistoryRecord,
  HistorySchema,
  type JobMode,
...
function jobId(startedAt: string, mode: JobMode): string {
 ...
 */
function reapStaleLock(dataDir: string): HistoryRecord | null {
