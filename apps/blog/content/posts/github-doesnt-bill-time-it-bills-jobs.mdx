...
## The comment that used to be free

This was in the copied workflow, and I wrote it:

```yaml
# Runs are never cancelled: they seed the caches, and a dispatched run is one
# someone asked for on purpose. Two dispatches close together are two runs.
concurrency:
  group: ${{ github.workflow }}-${{ github.ref }}
  cancel-in-progress: false
```

`cancel-in-progress: false` is deliberate, reasoned, and documented. It is also the single line most responsible for the shape of that Thursday. An agent working through a task pushes corrections in bursts — four pushes inside eleven seconds, in one case. With cancellation off, that is four complete gates, forty-eight jobs, forty-eight minutes of floor time, and three of the four were obsolete before they finished.

Twelve of 189 runs were canceled, and those were canceled by hand.

I want to be precise about what went wrong here, because "you forgot `cancel-in-progress`" is not it. I did not forget. I made the decision, for a reason I can still defend, in a context where the decision was free. The failure was that the decision moved to a context where it was not free, **and nothing in the copy asked me to re-read it.** Conventions travel as text. The conditions that justified them do not travel at all.
...
It is this. A pipeline encodes decisions about speed, thoroughness and cost, and only two of the three are visible in the file. Splitting a job to go faster, leaving cancellation off to warm a cache, running a matrix wide instead of deep — every one of those has a price attached, and in a public repository the price is multiplied by zero, so you can make hundreds of them without ever forming an opinion about any of them.
