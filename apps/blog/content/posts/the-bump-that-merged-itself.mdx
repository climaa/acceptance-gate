...
I expected that model to break on a dependency bot. It does not. It generalises, and the
generalisation is less comfortable than I would like.
...
behaviour and complexity. It is not evidence that the bump is a good idea, and it is not
...
policy-sync test and a mass rebaseline together, so it is a piece of work with an issue
...
No type checker sees that. Nothing in the lockfile hints at it. It is a behavioural break
...
runtime. A latent licence is harder to notice than a failure.

I have asked a version of this question before. [The migration that passed
CI](/blog/the-migration-that-passed-ci) ends on it: if every colour in the app shifted one
...
correctly declined, and reported the same colour it reports when it succeeds.
