...
Until this week, both production apps handled errors with a themed fallback screen, politely rendered, and no reports. Error boundaries [landed in #400](https://github.com/climaa/acceptance-gate/pull/400) and did their visual job. If triggered in production, only the visitor knew.
...
The implementation followed the usual pipeline: two issues released to [Sandcastle](/blog/what-is-sandcastle), handled in a single run. The first issue made error boundaries from #400 into the logger's first call sites using `useEffect` on the `error` prop. The second built the Bugsink adapter. Implementer and reviewer agents handled both; the wave took just under two hours.
