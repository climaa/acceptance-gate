...
describe('parseOverrideLabels', () => {
  ...
  it('reads a model alias for a per-issue role', () => {
    // Arrange & Act
    const { overrides, errors } = parseOverrideLabels(['sc:implementer:sonnet-5']);
  ...
  it('reads an effort override independently of the model', () => {
    // Arrange & Act
    const { overrides, errors } = parseOverrideLabels(['sc:reviewer:effort:xhigh']);
  ...
  it('accepts a canonical model id as well as an alias', () => {
    // Arrange & Act
    const { overrides, errors } = parseOverrideLabels([`sc:implementer:${SONNET}`]);
  ...
  it('rejects an unknown model rather than passing it to the CLI', () => {
    ...
    const { overrides, errors } = parseOverrideLabels(['sc:implementer:sonnnet-5']);
...
describe('mergeProfile — precedence is PROFILES < env < label', () => {
  ...
  it('merges model and effort from different layers', () => {
    // Arrange & Act
    const { profile } = mergeProfile(BASE, { effort: 'low' }, { model: SONNET });
