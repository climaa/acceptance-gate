...
| `apps/manual` — the console's manual, generated from its acceptance suite | ✅ [published](https://acceptance-gate-manual.vercel.app) — three pages (console, report, sample) parsed from the same `.feature` files the acceptance suite runs, so a scenario added or removed changes the manual in the same commit; page views tracked on the same terms as the blog and the console                                                                                                          |
