...
| `apps/e2e` — playwright-bdd acceptance suite                    | ✅ runs on every PR, blocks the merge — in `gate.needs`. Its second lane, `features/local/`, never runs in CI and writes to your own `.visual-diff` — see [the two `:local` scripts](#the-two-local-scripts-capture-promote-and-clean-up-after-themselves)                                                                                                     |
...
tree**, and the one scenario in that lane writes: it captures your whole corpus
inside the pinned container, compares it, promotes it, and then **removes every
one of those again**. It needs a running Docker daemon and takes minutes.

It costs you nothing, which is the part worth knowing: it captures the set it
needs rather than borrowing one of yours, so it starts on a console that has
captured nothing and leaves one behind. Nothing of yours is deleted, and the lane
