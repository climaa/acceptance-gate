...
| `apps/visual-diff-ui` — the console that reviews those runs     | ✅ [published](https://acceptance-gate-visual-diff-ui.vercel.app) — the visual-diff review console: sets, reports and history, the run panel with its live log and the container-bound accept gate; sample mode browses the #242 regression report |
