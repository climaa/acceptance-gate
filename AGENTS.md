...
| `turbo.json`                             | The task edges (`dependsOn: ["^build"]`) and each task's `inputs`                          | Strict JSON, no comments — the task graph and cache inputs are load-bearing for the gate's correctness                                |
...
CSS shimmer will be the one sanctioned exception, arrives with Wave 1).
